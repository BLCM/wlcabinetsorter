...
class ModFile(Cacheable):
    ...
    def __init__(self, mtime, dirinfo=None, filename=None, initial_status=Cacheable.S_UNKNOWN,
        ...
        if filename is not None and filename.lower().endswith('.wlpakinfo'):
        ...
        if dirinfo:
            ...
            # base Wonderlands objects which are latin1, so we can't
    ...
    def _unserialize(self, input_dict):
        ...
        # Added 2023-01-25
        if self.full_filename is not None and self.full_filename.lower().endswith('.wlpakinfo'):
            self.is_pak_only = True
        else:
            self.is_pak_only = False
    ...
    def find_matching(self, mod_name, single_mod=True):
        ...
        if single_mod:
            ...
            for section in self.mapping.keys():
                if Levenshtein.ratio(mod_name_lower, section) > .8:
                    return self.mapping[section]
            # New for the BL3/WL modcabinet, since I've got multiple variants of
...
class App(object):
    ...
    categories = collections.OrderedDict([
        ...
        ('chaos', Category('General Gameplay and Balance: Chaos Mode Changes')),
        ('encounters', Category('General Gameplay and Balance: Encounter Changes')),
        ...
        # Classes and Skills
        ('class-overhaul', Category('Classes and Skills: Full Class Overhauls')),
        ('skill-system', Category('Classes and Skills: Skill System Changes')),
        ('class-brrzerker', Category('Classes and Skills: Brr-Zerker Changes')),
        ('class-clawbringer', Category('Classes and Skills: Clawbringer Changes')),
        ('class-graveborn', Category('Classes and Skills: Graveborn Changes')),
        ('class-spellshot', Category('Classes and Skills: Spellshot Changes')),
        ('class-sporewarden', Category('Classes and Skills: Spore Warden Changes')),
        ('class-stabbomancer', Category('Classes and Skills: Stabbomancer Changes')),
        ('class-other', Category('Classes and Skills: Other Class Changes')),

        # Weapons/Gear
        ('gear-general', Category('Weapons/Gear: General')),
		('gear-enchantments', Category('Weapons/Gear: Enchantments')),
        ...
        ('gear-ring', Category('Weapons/Gear: Rings')),
        ('gear-amulet', Category('Weapons/Gear: Amulets')),
    ...
    def __init__(self, ini_file):
        ...
        self.log_file = os.path.join(self.log_dir, 'wlcabinetsorter.log')
    ...
    def _run(self,
        ...
        reserved_pages.add('Wonderlands Mods.md')
        ...
        for (dirpath, dirnames, filenames) in os.walk(game_dir):
            ...
            processed_files = []
            for mod_file in dirinfo.get_all_with_ext('wlhotfix') \
                    + dirinfo.get_all_with_ext('wlhotfix.gz') \
                    + dirinfo.get_all_with_ext('wlpakinfo'):
        ...
        self.write_wiki_file(wiki_files,
                authors_filename,
                self.authors_template.render({
                    'authors': sorted(self.author_cache.items(), key=lambda t: t[0].lower()),
                    })
                )

        # Write out the Home page, linking to all categories which have mods
        self.logger.debug('Writing out Home page')
        self.write_wiki_file(wiki_files,
                home_filename,
        ...
        game_filename = 'Wonderlands Mods.md'
        ...
        if do_git and do_git_commit:
            ...
            if wikirepo.is_dirty():
                ...
                wikirepo.git.commit('-a', '-m', 'Auto-update from wlcabinetsorter')
                wikirepo.git.push()
            else:
